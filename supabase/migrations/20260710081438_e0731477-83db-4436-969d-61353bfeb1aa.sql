...
BEGIN
  ...
  -- Only solving a practice question advances the streak.
  -- Other activity types (login, mock_test_attempted, etc.) are still
  -- logged for history but do not move the streak counters.
  _counts_toward_streak := _activity_type = 'practice_question_solved';
  ...
  IF NOT _counts_toward_streak THEN
    -- Log the activity but do not touch the streak counters.
