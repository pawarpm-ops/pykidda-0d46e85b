...
BEGIN
  ...
  -- Any meaningful learning activity counts toward the daily streak:
  -- opening OR completing homework / practice / mock.
...
-- Backfill: siddhu solved a practice question today but got streak 0. Fix retroactively.
UPDATE public.student_streaks s
SET current_streak = 1,
    longest_streak = GREATEST(longest_streak, 1),
    last_activity_date = (now() AT TIME ZONE 'Asia/Kolkata')::date,
    today_completed = true
WHERE user_id IN (
  SELECT DISTINCT l.user_id
  FROM public.streak_activity_logs l
  WHERE l.activity_date = (now() AT TIME ZONE 'Asia/Kolkata')::date
    AND l.activity_type IN ('practice_question_solved','homework_submitted','mock_test_attempted')
)
AND (s.last_activity_date IS NULL OR s.last_activity_date < (now() AT TIME ZONE 'Asia/Kolkata')::date);