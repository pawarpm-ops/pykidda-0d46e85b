// AI mock test — warning / pre-flight page. Mirrors the built-in secure warning
// so AI tests follow the same anti-cheat + keyboard-only rules as normal mock tests.
...
export const Route = createFileRoute("/mock-tests/ai/$testId/warning")({
  head: () => ({
    meta: [
      { title: "Secure Keyboard-Only AI Mock Test · PY Kidda" },
...
function Warning() {
  ...
  useEffect(() => {
    ...
    const tooSmall =
      typeof window !== "undefined" && (window.innerWidth < 900 || window.innerHeight < 600);
    const noFs =
    ...
    if (isTouchOnly || tooSmall) {
      setUnsupported(
        "Mock tests can only be taken on a desktop or laptop with a physical keyboard and mouse. Please switch to a laptop/desktop in fullscreen mode.",
      );
    } else if (noFs) {
      setUnsupported(
        "Your browser does not support secure fullscreen mode. Please use the latest Chrome, Edge, or Firefox on desktop.",
      );
    }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-6">
        <div className="max-w-lg rounded-xl border border-destructive/40 bg-destructive/10 p-6 text-center">
          ...
          <Link to="/mock-tests" className="mt-4 inline-block underline text-sm">
            Back to mock tests
          </Link>
        </div>
      </div>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-6">
        <div className="max-w-lg rounded-xl border-2 border-destructive/50 bg-destructive/10 p-6 text-center">
          ...
          <Link
            to="/mock-tests"
            className="mt-5 inline-block rounded-md border border-border bg-background px-4 py-2 text-sm font-medium"
          >
            Back to mock tests
          </Link>
        </div>
  ...
  async function startTest() {
    ...
    try {
      ...
      /* if denied, run page will detect and auto-submit */
    }
    navigate({ to: "/mock-tests/ai/$testId/take", params: { testId } });
  }
      ...
      <div className="mx-auto w-full max-w-5xl">
        <div className="text-center">
          ...
          <h1 className="mt-2 text-3xl md:text-4xl font-extrabold leading-tight bg-gradient-to-r from-accent via-primary to-accent bg-clip-text text-transparent">
            Secure Keyboard-Only AI Mock Test Mode
          </h1>
        ...
        <div className="mt-6 rounded-xl border-2 border-destructive/50 bg-destructive/10 p-5 text-sm leading-relaxed">
          <p className="font-bold text-destructive uppercase tracking-wide text-xs">
        ...
        <div className="mt-6 rounded-xl border border-border bg-card p-5">
          ...
          <p className="font-bold text-lg mt-1">{test.title}</p>
          <p className="text-sm text-muted-foreground">
            {test.question_count} questions · {Math.round(test.duration_sec / 60)} minutes ·{" "}
            {test.total_marks} marks · AI-generated
          </p>
          {test.description && (
            <p className="mt-2 text-sm text-foreground/80">{test.description}</p>
          )}
        </div>
