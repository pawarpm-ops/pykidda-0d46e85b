      ...
      <main className="mx-auto max-w-5xl px-6 py-12">
        ...
        <section className="mt-8">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-semibold">Teacher-published AI tests</h2>
          </div>
          {loading ? (
            <p className="mt-3 text-sm text-muted-foreground">Loading…</p>
          ) : aiTests.length === 0 ? (
            <p className="mt-3 text-sm text-muted-foreground">No AI tests published yet — check back soon!</p>
          ) : (
            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              {aiTests.map((t) => (
                <div key={t.id} className="rounded-xl border border-border bg-card p-5 flex flex-col relative overflow-hidden">
                  <div className="absolute top-0 right-0 rounded-bl-lg bg-[oklch(0.65_0.16_145)] px-2 py-1 text-[10px] font-bold uppercase tracking-widest text-white">AI</div>
                  <h3 className="font-semibold text-lg pr-12">{t.title}</h3>
                  <p className="mt-2 text-sm text-muted-foreground flex-1">{t.description || "AI-generated mock test."}</p>
                  <div className="mt-4 flex items-center justify-between text-sm">
                    <span className="text-muted-foreground">
                      {t.question_count} Qs · {Math.round(t.duration_sec / 60)} min · {t.total_marks} marks
                    </span>
                    <Link
                      to="/mock-tests/ai/$testId/take"
                      params={{ testId: t.id }}
        ...
        <section className="mt-10">
          <div className="flex items-center gap-2">
            <span className="text-lg">📘</span>
            <h2 className="font-semibold">Built-in coding practice tests</h2>
          </div>
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            {MOCK_TESTS.map((t) => {
